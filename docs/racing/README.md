...
which has no dependencies beyond the Python standard library. Run it to
regenerate them all.

The plotted figures use parameter values that are **plausible for a 1/10-scale
car and have not been measured on one**. They illustrate shapes and
relationships, and no number read off them means anything about a real
vehicle. The schematic figures are geometry and carry no model at all. The
