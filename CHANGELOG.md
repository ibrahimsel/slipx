...
- **A second generated track ships, and the demo grid races** (ADR-0057).
  ...
  1.4 m, a hairpin behind the widest braking zone on the lap, 100.891 m a
  lap, and the first shipped track whose width varies along the lap.
  Generated by the same asserted, standard-library
  `examples/tracks/make_tracks.py`; the stadium's bytes are unchanged and
  CI still asserts against the stadium. The ROS demo driver grows two
  modes on top of `gap` and `pursuit`: `racer` is pure pursuit that
  watches the scan and leans past traffic instead of lifting, and
  `mixed` deals a seeded field of racers and gap followers with spread
  top speeds, lookaheads and aggression, printed one line per car.
  `watch_a_race.sh` defaults to the mixed field on the circuit. Article
  21 explains what a pass costs.
